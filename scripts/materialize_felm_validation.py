...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run FELM materialization in a Modal GPU sandbox.")
    ...
    parser.add_argument("--log-path", type=Path, default=Path("artifacts/runs/felm_wk_materialization_remote.log"))
    ...
    cmd = [
        ...
        "--",
        "python",
        "scripts/remote_felm_materialize.py",
        "--examples-jsonl",
    ...
    if result.returncode != 0:
        raise SystemExit(
            "Remote FELM materialization failed. "
    ...
    for row in raw_self_reports:
        (args.raw_dir / f"{row['example_id']}.txt").write_text(row["raw_text"], encoding="utf-8")
    write_jsonl(args.parsed_jsonl, parsed_self_reports)

    npz = np.load(args.features_npz, allow_pickle=False)
    print(f"Wrote FELM materialization summary to {args.summary_json}")
