...
def _batch_complete(summary_path: Path, expected_segments: int) -> bool:
    ...
    try:
        ...
        return False
    return (
        summary.get("n_expected_segments") == expected_segments
        and summary.get("n_feature_rows") == expected_segments
        and summary.get("n_tokenized_segments") == expected_segments
        and summary.get("segment_failure_rate") == 0
    )
...
def _run_batch(args: argparse.Namespace, batch_dir: Path, start: int, end: int) -> None:
    cmd = [
        ...
        "run",
        "python",
        "scripts/materialize_felm_validation.py",
        "--examples-jsonl",
        str(args.examples_jsonl),
        "--segments-jsonl",
        str(args.segments_jsonl),
        "--answer-runs-jsonl",
...
def _merge_batches(args: argparse.Namespace, batch_dirs: list[Path]) -> dict[str, Any]:
    ...
    np.savez_compressed(
        args.features_npz,
        matrix=matrix,
        claim_ids=np.asarray(claim_ids, dtype=str),
        example_ids=np.asarray(example_ids, dtype=str),
    )

    expected_segments = sum(int(summary.get("n_expected_segments", 0)) for summary in summaries)
    ...
    merged_summary = {
        ...
        "n_expected_segments": expected_segments,
        "n_tokenized_segments": sum(int(summary.get("n_tokenized_segments", 0)) for summary in summaries),
        ...
        "segment_failure_rate": 0.0 if expected_segments == 0 else 1.0 - (feature_rows / expected_segments),
        "self_report_failure_rate": (
            0.0 if expected_segments == 0 else 1.0 - (self_report_predictions / expected_segments)
        ),
        "transport_note": "Full FELM materialization is batched to avoid large stdout payload truncation.",
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Materialize FELM validation artifacts in small Modal batches.")
    parser.add_argument("--examples-jsonl", type=Path, default=Path("data/felm/felm_wk_examples.jsonl"))
    parser.add_argument("--segments-jsonl", type=Path, default=Path("data/felm/felm_wk_segments.jsonl"))
    parser.add_argument("--batch-dir", type=Path, default=Path("artifacts/runs/felm_wk_materialization_batches"))
    parser.add_argument("--batch-size", type=int, default=20)
    parser.add_argument("--answer-runs-jsonl", type=Path, default=Path("artifacts/runs/felm_wk_answer_runs.jsonl"))
    parser.add_argument("--tokenized-claims-jsonl", type=Path, default=Path("artifacts/runs/felm_wk_claims_tokenized.jsonl"))
    parser.add_argument("--features-npz", type=Path, default=Path("artifacts/runs/felm_wk_probe_features_residual.npz"))
    parser.add_argument("--raw-dir", type=Path, default=Path("data/cached_baselines/llama_self_report/raw/felm_wk"))
    parser.add_argument("--parsed-jsonl", type=Path, default=Path("data/cached_baselines/llama_self_report/parsed/felm_wk/_all_predictions.jsonl"))
    parser.add_argument("--summary-json", type=Path, default=Path("artifacts/runs/felm_wk_materialization_summary.json"))
    ...
    segments_by_example: dict[str, int] = {}
    for row in read_jsonl(args.segments_jsonl):
        segments_by_example[row["example_id"]] = segments_by_example.get(row["example_id"], 0) + 1
    ...
    for batch_index, start in enumerate(range(0, len(examples), args.batch_size)):
        ...
        expected_segments = sum(segments_by_example.get(row["example_id"], 0) for row in batch_examples)
        batch_dir = args.batch_dir / f"batch_{batch_index:04d}_{start:04d}_{end:04d}"
        batch_dir.mkdir(parents=True, exist_ok=True)
        batch_dirs.append(batch_dir)
        if not args.force and _batch_complete(batch_dir / "summary.json", expected_segments):
            print(
                f"Reusing completed FELM batch {batch_index + 1}/{len(range(0, len(examples), args.batch_size))}: {start}:{end}",
                flush=True,
            )
            continue
        print(f"Running FELM batch {batch_index + 1}/{len(range(0, len(examples), args.batch_size))}: {start}:{end}", flush=True)
        _run_batch(args, batch_dir, start, end)

    summary = _merge_batches(args, batch_dirs)
    print(f"Wrote merged FELM summary to {args.summary_json}")
    print(f"Wrote merged residual feature matrix to {args.features_npz}")
    print(
        "Merged counts: "
        f"{summary['n_examples']} examples, {summary['n_feature_rows']}/{summary['n_expected_segments']} feature rows, "
        f"{summary['n_self_report_predictions']}/{summary['n_expected_segments']} self-report predictions"
    )
